...
def _save_current_block_data():
    ...
    st.session_state.historical_blocks[st.session_state.latest_block_number] = st.session_state.current_block_data
    st.session_state.current_block_data = BlockData()
...
if st.sidebar.button(":chart_with_upwards_trend: Pending Block"):
    st.session_state.current_view = "pending_block"
st.sidebar.title(body=f"Currently viewing: {'Mempool' if st.session_state.current_view == 'pending_block' else st.session_state.current_view}")
...
async def process_data_stream():
    ...
    try:
        ...
        st.error(f"Error in process_data_stream: {type(e).__name__}, {str(e)}")
        await asyncio.sleep(1)  # 
        st.error("Restarting data stream...")
